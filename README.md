...
欢迎贡献插件，进行pull request即可。（部分系统仍在开发中，现在可以提前开发插件，并使用机器人）

目前暂未开放服务端下载，适配器已适配了QQ平台（Mirai）、Telegram、WeChat。未来将会开放相应服务端+平台适配器微服务程序下载，在自己的服务器启动上并选用安装所需要的插件，就可以得到属于自己的机器人了。
...
一般开发后的插件命令都与alias插件（命令别名）进行配合，将其变得更容易调用，甚至可以通过alias插件设置正则表达式，能够使用自然语言式的调用。
