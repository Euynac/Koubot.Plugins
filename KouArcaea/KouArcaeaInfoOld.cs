...
//    public class KouArcaeaInfo : KouPlugin<KouArcaeaInfo>, IDisposable
//    {
...
//        {
//            if (_kouContext.Set<Song>().IsNullOrEmptySet()) return "曲库为空";//BUG 更新后无法进行判断是否为空
//            if (DelegateExtensions.SatisfyAll(string.IsNullOrWhiteSpace, name, ChartConstant,
...
//            List<Song> satisfiedSongs = GetSatisfiedSong();
...
//            try
//            {
//                if (_kouContext.Set<Song>().IsNullOrEmptySet()) return "曲库为空";
...
//                Song.RatingClass ratingClass = Song.RatingClass.Random;
//                //支持一个定数信息
...
//        /// <returns></returns>
//        public List<Song> GetSatisfiedSong(List<string> ratingList = null, Song.RatingClass ratingClass = Song.RatingClass.Random)
//        {
//            if (_kouContext.Set<Song>().IsNullOrEmptySet()) return null;
//            //处理限定难度信息
...
//                {
//                    if (Song.RatingClassNameList.ContainsKey(RatingClass.ToLower()))
//                    {
//                        ratingClass = Song.RatingClassNameList[RatingClass.ToLower()];
//                    }
//                }
//            }
//            //设定难度类型默认值为FTR
...
//            {
//                ratingClass = Song.RatingClass.Future;
//            }
//            //处理歌名信息
//            if (!SongName.IsNullOrWhiteSpace())
//            {
//                SongName = SongName.Trim().ToLower();
//                string songEnID = _kouContext.Set<SongAlias>().FirstOrDefault(s => s.Alias == SongName)?.SongEnId;
//                if (songEnID != null)
//                {
//                    SongEnID = songEnID;
//                    SongName = null;
...
//            //将需要用到的过滤器放到过滤容器里
//            FilterContainer<Song> filterContainer = new FilterContainer<Song>();
//            //使用默认ModelFilter
//            filterContainer.AddAutoModelFilter(nameof(Song.SongTitle), SongName, FilterType.Default, SortType.Ascending);//增加Song_title字段的默认filter
//            filterContainer.AddAutoModelFilter(nameof(Song.SongEnId), SongEnID, FilterType.Exact);
//            if (ratingClass != Song.RatingClass.Random) filterContainer.AddAutoModelFilter(nameof(Song.ChartRatingClass), ratingClass, FilterType.SupportValueDefault);
//            filterContainer.AddAutoModelFilter(nameof(Song.SongArtist), SongArtist, FilterType.Default, SortType.Ascending);
//            filterContainer.AddAutoModelFilter(nameof(Song.ChartDesigner), ChartDesigner, FilterType.Default, SortType.Ascending);
//            filterContainer.AddAutoModelFilter(nameof(Song.JacketDesigner), JacketDesigner, FilterType.Default, SortType.Ascending);
//            filterContainer.AddAutoModelFilter(nameof(Song.ChartAllNotes), NotesCount, FilterType.Default, SortType.StringAuto);
//            filterContainer.AddAutoModelFilter(nameof(Song.ChartConstant), ChartConstant, FilterType.Default, SortType.StringAuto);
//            filterContainer.AddAutoModelFilter(nameof(Song.SongLength), SongLength, FilterType.Default, SortType.StringAuto);
//            filterContainer.AddAutoModelFilter(nameof(Song.SongBpm), SongBPM, FilterType.Interval, SortType.StringAuto);
//            filterContainer.ActivateAutoModelFilter();
//            //加入自定义的filter
//            if (!ratingList.IsNullOrEmptySet()) filterContainer.Add(FilterContainer<Song>.Convert(Song.RatingNumFilter), ratingList);


//            //开始筛选
//            var selectedList = _kouContext.Set<Song>().ToList();
...
//        /// <returns></returns>
//        public Song RandomGetOneSong(List<string> ratingList = null, Song.RatingClass ratingClass = Song.RatingClass.Future)
//        {
...
//        /// <returns></returns>
//        public List<Song> RandomGetSong(List<string> ratingList = null, Song.RatingClass ratingClass = Song.RatingClass.Future, int count = 1)
//        {
...
//        /// <returns></returns>
//        public string AutoFormatSong(Song song)
//        {
...
//        {
//            string songEnID = _kouContext.Set<SongAlias>().Where(name => name.Alias == songName).FirstOrDefault()?.SongEnId;
//            if (songEnID == null)
//            {
//                var songs = _kouContext.Set<Song>().Where(s => s.SongTitle.Contains(songName) && s.ChartRatingClass == Song.RatingClass.Future);
//                if (songs.Count() == 0) return $"找不到是哪个歌叫做{SongName}";
//                if (songs.Count() > 1) return $"具体是指哪首歌可以叫{songAnotherName}？\n{songs.ToSetString<Song>()}";
//                songEnID = songs.First().SongEnId;
//            }
//            var allRatingSongs = _kouContext.Set<Song>().Where(s => s.SongEnId == songEnID);


//            if (_kouContext.Set<SongAlias>().Any(x => x.Alias == songAnotherName && x.SongEnId == songEnID)) return $"我之前就知道能叫{songAnotherName}了";
//            var learnedList = _kouContext.Set<SongAlias>().Where(x => x.SongEnId == songEnID).ToList();
//            string anotherNames = "";
//            if (learnedList.Count > 0)
//            {
//                foreach (var item in learnedList)
//                {
//                    anotherNames += item.Alias + $"({item.AliasID})、";
//                }
//                anotherNames = anotherNames.TrimEnd('、');
//            }

//            SongAlias aliasModel = new SongAlias
//            {
//                Alias = songAnotherName,
//                SongEnId = songEnID,
//            };
...
//            {
//                return $"学到许多，{allRatingSongs.First().SongTitle}可以叫做{songAnotherName}({aliasModel.AliasID})" + (anotherNames.IsNullOrWhiteSpace() ? null : $"，我还知道它能叫做{anotherNames}！");
//            }
//            return "啊...不知道为什么没学会";
...
//            {
//                songAlias = _kouContext.Set<SongAlias>().SingleOrDefault(x => x.AliasID == ID);
//            }
//            else if (_kouContext.Set<SongAlias>().Any(x => x.Alias == NameOrID))
//            {
//                var names = _kouContext.Set<SongAlias>().Where(x => x.Alias == NameOrID);
//                if (names.Count() > 1) return $"具体是删除哪个？\n{names.ToSetString<SongAlias>()}";
//                songAlias = names.First();
//            }
//            else return "我没学过这个的吧";
//            _kouContext.Remove(songAlias);
