...
namespace KouArcaea.Room;

public class ArcaeaImageGuessLeaderBoard : IGameRoomLeaderBoardInfo<ArcaeaImageGuessLeaderBoard>
{
    ...
    public int CompareTo(ArcaeaImageGuessLeaderBoard other)
    {
...
public class ArcaeaImageGuessGameRoom : KouGameRoom<ArcaeaImageGuessLeaderBoard>
{
    public const string Help = "Arcaea歌曲猜图，每轮不定时放出随机裁切过的歌曲图片，玩家需要猜出图片对应的歌曲名称或别名，最先猜对的计分。开始游戏后，默认10分钟后结算成绩。每10轮难度升级（包含缩小、翻转、旋转、灰度等），并增益15%奖池。";

    public static Lazy<HashSet<int>> NotExistSet { get; set; } = new(() =>
    {
        var list = new List<int>();
        foreach (var (url, id) in Song.GetCache()!.Select(p => (p.JacketUrl, p.SongId)))
        {
    ...
    public Song CurAnswer { get; set; }
    public ArcaeaImageGuessGameRoom(string roomName, PlatformUser ownerUser, PlatformGroup roomGroup, int? fee) : base(roomName, ownerUser, roomGroup, fee)
    {
        RoomHelp = Help;
    ...
    public override RoomReaction PromptGetPrompt(PlatformUser speaker, string content)
    {
        ...
        var promptList = new List<string?>
        {
            ...
            CurAnswer.SongPack?.Be($"\n曲包：{CurAnswer.SongPack}"),
    ...
    public override void NewRound(bool isRenew = false)
    {
        ...
        var info = Song.GetCache()!.Where(p=>!hash.Contains(p.SongId)).ToList().RandomGetOne();
        if (info == null) return;
        CurAnswer = info;
        CurImage = null;
        var image = new KouImage(info.JacketUrl, new Song());
        ...
        var rank = (4 - RoundCount / 10).LimitInRange(1, 4);
    ...
    {
        var list = Song.Find(p =>
            p.Aliases.Any(a => a.Alias.Equals(aliasOrName, StringComparison.OrdinalIgnoreCase)));
        if (list.Count == 1 && list[0].Equals(CurAnswer)) return 1;
        var idList = Song.Find(p => p.Aliases.Any(a =>
                                             a.Alias.Contains(aliasOrName, StringComparison.OrdinalIgnoreCase)) ||
                                         p.SongTitle.Contains(aliasOrName,
                                             StringComparison.OrdinalIgnoreCase))
            .Select(p => p.SongId).Distinct().ToList();
    ...
    public override RoomReaction PromptGiveUp(PlatformUser speaker, string content)
    {
        ...
        NewRound(true);
        return
            $"已放弃该轮，答案公布：{previous.SongTitle}\n{new KouImage(previous.JacketUrl, new Song()).ToKouResourceString()}";
    }
